...
[GitHub Releases](https://github.com/marketintellabs/hermes-paperclip-adapter/releases)
...
opts in). See [`src/server/openrouter-mode.ts`](./src/server/openrouter-mode.ts).
...
delegation. New [`src/mcp/tools/priority.ts`](./src/mcp/tools/priority.ts)
...
- **MCP tool server** (`builtin:mil-heartbeat-v3`, 0.7.0+) — the
  `paperclip-mcp` stdio server that replaced curl-in-prompt. See
...
| `paperclipMcpTools` | string[] | *(all)* | Per-agent MCP tool allowlist. `[]` = deny-all; absent = register every tool. See [`CHANGELOG.md`](./CHANGELOG.md) (0.8.8-mil.0) for the design rationale. |
