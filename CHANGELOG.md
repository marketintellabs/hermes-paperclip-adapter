...
## [0.9.1-mil.0] — UNRELEASED — LLM-API failure classifier (depends on 0.9.0-mil.0 from PR #28)

### Added
- **Structured classification of LLM-provider failure modes — promotes them from raw text in `result_json.result` to first-class telemetry.** New `src/server/llm-error-classifier.ts` inspects each run's final-message text (and `executionResult.errorMessage` as a secondary source) for known LLM-API failure signatures and emits a stable `errorCode` plus an operator-actionable one-liner that says exactly what to do next. Patterns covered (every regex anchored on a real production failure harvested in the 2026-05-02 fleet audit, never speculation): **HTTP 402** budget / credit exhaustion → `provider_budget_exhausted` → "<provider> returned HTTP 402 — top up the account or raise the daily limit on the key. If this fires repeatedly across a single day, check whether one agent's max_tokens is set too high relative to remaining headroom."; **HTTP 401 / 403** auth → `provider_auth_failed` → "<provider> rejected the API key — rotate <PROVIDER>_API_KEY via Secrets Manager and force-deploy the affected hermes ECS service."; **HTTP 429** rate-limit → `provider_rate_limited` (NOT marked dead — the next wake may succeed); **"failed after N retries"** generic fallback → `llm_call_exhausted_retries` → "<provider> LLM call exhausted Hermes' retry budget without resolving — check provider status / network egress, or grep run output for the underlying error code." Provider name is text-derived first (URL signatures: `openrouter.ai`, `api.anthropic.com`, `api.openai.com`, `api.deepinfra.com`/`huggingface.co`) and falls back to the adapter's `resolvedProvider` so the hint stays accurate even when Hermes routes through a meta-router that differs from what `resolvedProvider` reports.
- **Wire-up in `src/server/execute.ts`** between the auto-repair detection block and the existing `liveness.recordBeat("run_end")` finalization. When a classification fires, the adapter sets `resultJson.errorCode`, sets `resultJson.errorEvidence` (capped 240 chars — operator-readable quote of the matched substring), calls `liveness.recordHint(hint)` so the actionable next step lands in `result_json.nextActionHints[]`, and (only for `provider_budget_exhausted` / `provider_auth_failed` / `llm_call_exhausted_retries`) calls `liveness.markDead(errorCode)` so the run lands as `livenessState=dead` instead of falling through with no liveness signal at all. `provider_rate_limited` instead calls `markStalled()` because the next wake may legitimately succeed; we reserve `dead` for verdicts where the operator must intervene before any agent on this provider can recover. A single `[hermes] LLM-API failure classified: <code> ...` stderr line is also emitted at classification time for grep-friendly log analysis.
- **18 new unit tests** in `llm-error-classifier.test.ts` covering: the literal real-world OpenRouter 402 blob harvested from the CEO's 2026-05-01T21:01:01 run, provider detection from URL signatures, fallback to caller-supplied provider when no URL is present, the alternate "create a key with a higher daily limit" phrasing without HTTP 402 in the same line, HTTP 401/403 auth (with provider-aware env-var hint: `OPENROUTER_API_KEY` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `HF_TOKEN`), an `invalid_api_key` JSON body without a status code, HTTP 429 with `dead=false` semantics + the lowercase `rate-limited` and "too many requests" alternate phrasings, the generic "failed after N retries" fallback, **the precedence test** (a real blob containing both "failed after 3 retries" AND "HTTP 402" must classify as `provider_budget_exhausted`, not the generic fallback), null/empty/non-string defensive handling, clean successful response → null, MCP-tool-error JSON body → null (we deliberately don't double-classify what the per-tool retry logic already handles), and the 240-char evidence cap. Total adapter test count: 373 → 391.
...
- **Solves a real production health gap, not a speculative one.** The 2026-05-02 fleet audit found 118 of the most recent 200 runs in the failed/timed-out state, with the dominant cause being OpenRouter HTTP 402 budget exhaustion across CEO (3 wakes), Investigative Correspondent (3 wakes), Managing Editor (3 wakes), and others — all silently bleeding into `result_json.result` as raw text, none of them surfacing the actionable "raise the daily limit" path to the operator. After this release, the same failure mode lands as `errorCode=provider_budget_exhausted` + `nextActionHints=["openrouter returned HTTP 402 (budget/credit exhausted) — top up the account or raise the daily limit on the key. ..."]` so dashboards and a future Staff Engineer agent can filter / aggregate / alert on the structured signal.
- **Sequencing — depends on 0.9.0-mil.0 from PR #28 landing first.** This branch is cut from `feat/0.9.0-mil-heartbeat-v4`, so the `mil-heartbeat-v4` template + resolver wiring are inherited. Merge order: PR #28 → main → publish 0.9.0-mil.0 to npm → tag → THIS PR (#29) rebases cleanly onto main → merge → publish 0.9.1-mil.0 to npm. The MIL-side fleet flip can pin to whichever is the latest tag at flip-time.

[Draft release notes →](https://github.com/marketintellabs/hermes-paperclip-adapter/pull/29)

## [0.9.0-mil.0] — UNRELEASED — DRAFT (Gate A passed via forced LinkedIn Strategist test on 2026-05-02; ready-for-review at PR #28)

### Added
- **New builtin prompt template `mil-heartbeat-v4`** (`templates/mil-heartbeat-v4.md`). Same adapter-owned status semantics and same in-process `paperclip-mcp` tool server as `mil-heartbeat-v3`, but additionally: (a) retires the `RESULT:` marker from the prompt surface — the parser still honours it as a server-side fallback for any v3-pinned agent during the v3→v4 rollout window, but v4 agents are no longer taught it, so the LLM is funnelled toward `mcp_paperclip_update_issue_status` as the canonical completion signal; (b) documents the BETA `mcp_paperclip_post_issue_interaction` tool with a worked example for each of the three currently-shipping kinds (`request_confirmation` for single yes/no decisions like publish-vs-hold approvals, `suggest_tasks` for delegator decompositions that benefit from operator review before fan-out, `ask_user_questions` for specific blocking clarifications); (c) explicitly documents when NOT to use `post_issue_interaction` (in-run progress, internal coordination, logging) so the LLM doesn't over-use the new surface and turn every comment into a structured card.
- **`mil-heartbeat-v4` is registered in `BUILTIN_PROMPT_TEMPLATES`, `ADAPTER_OWNED_STATUS_TEMPLATES`, and `MCP_TOOL_TEMPLATES`** in `src/shared/constants.ts` — opts the new template into adapter-owned status transitions AND the per-run `paperclip-mcp` tool server alongside v3. Both v3 and v4 are supported indefinitely so the MIL repo can flip agents one at a time and roll back individually if a prompt regression surfaces (no adapter-side rollback needed).
- **New unit test in `resolve-prompt-template.test.ts`** that resolves `builtin:mil-heartbeat-v4`, asserts each of the three documented `IssueInteraction` kinds appears verbatim in the rendered template, and asserts the `RESULT:` marker has been retired from the prompt surface (catches a regression where someone accidentally ships v3 content under the v4 name, which would silently undo the rollout's whole point). Existing "resolves all known builtin templates" loop extended to include v4. Total adapter test count: 372 → 373.

### Notes
- **No wire-format breakage, no Hermes-arg change, no MCP tool surface change.** The new template is opt-in per agent via `adapterConfig.promptTemplate = "builtin:mil-heartbeat-v4"`. Agents not flipped continue to render v3 content with identical run behaviour to 0.8.21.
- **PR 4 of the adapter-0.9 phased rollout — adapter-side scaffolding only.** This adapter release ships the v4 template + resolver wiring + version bump. The 39-agent fleet flip + Dockerfile alias bump + promotion of `post_issue_interaction` from per-agent override to fleet-wide allowlist all live on the MIL-repo side and ship in a separate PR that's gated on the LinkedIn Strategist pilot's day-7 verification (see `docs/HANDOFF_2026-05-02.md § Active calendar-gated soak windows / Gate A` in the MIL repo for the four-criterion verification + `paperclip/scripts/check-pilot.mjs` for the automated check). Releasing the adapter-side scaffolding ahead of the gate keeps the gate-passes-to-MIL-flip cycle short — when Gate A closes, the MIL fleet flip is just a Dockerfile alias bump + a `configure-agents.mjs` run.
- **RESULT-marker parser is intentionally retained server-side.** `src/server/result-marker.ts` continues to parse `RESULT: done` / `blocked` / `cancelled` markers in run output and apply them as a fallback completion signal. This serves three purposes during the rollout: (a) v3-pinned agents (38 of 39 today) keep working unchanged; (b) v4 agents that mistakenly emit a marker despite the prompt's "Do not emit" instruction don't dead-end; (c) when the fleet flip completes and stabilises for ≥30 days with zero marker emissions in `result_json.toolCalls[]`, the parser can be removed in a future release. There's no rush — the parser is small, well-tested, and harmless when no marker appears.

[Draft release notes →](https://github.com/marketintellabs/hermes-paperclip-adapter/pull/28)

## [0.8.21-mil.0] — 2026-05-02

### Added
- **`post_issue_interaction` MCP tool — opt-in writer for Paperclip v2026.428.0's structured `IssueInteraction` records (BETA upstream API).** New `src/mcp/tools/post-issue-interaction.ts` exposes Paperclip's `POST /api/issues/:id/interactions` endpoint as a typed MCP tool. Supports the three currently-shipping interaction kinds: `suggest_tasks` (proposed sub-issues with optional cost/effort hints), `ask_user_questions` (multi-question form with `text`/`single_select`/`multi_select`/`number`/`boolean`/`date` controls), and `request_confirmation` (publish vs. hold cards with `confirm_label` / `decline_label` / `risk_level`). Tool input schema mirrors upstream `createIssueThreadInteractionSchema` from `packages/shared/src/validators/issue.ts`, surfaces the persisted record as the structured tool result, and threads the run id through `sourceRunId` automatically (read from `process.env.PAPERCLIP_RUN_ID`, which `runChildProcess` already injects into the spawned MCP child) so the board UI can correlate every interaction back to the run that authored it. Idempotency-key support and optional `title` / `summary` / `continuationPolicy` fields land per the upstream payload contract. **The tool is registered in the global registry but NOT in any agent's default allowlist** — agents only get it when their `paperclipMcpTools` config includes the name explicitly. This is the BETA-API soak switch: a single Tier-2 pilot agent (LinkedIn Strategist) gets it in this release; the marker convention (`<!-- mode: test -->`, `RESULT: done`) continues to work unchanged for the other 38 agents until `0.9.0-mil.0` flips the fleet to v4 prompts and retires markers.
- **Per-tool error classification — fix-args / retry / abort.** The tool wraps every upstream call in a try/catch that classifies `PaperclipClientError` failures by HTTP status: `4xx` (validation, bad payload shape) → `retryPolicy=fix-args` and the upstream zod issue body is forwarded verbatim into the tool result text so the LLM can read which `payload.<field>` failed and self-correct on the next attempt; `5xx` and bare network errors → `retryPolicy=retry` (the existing per-tool retry helper takes over); `401` / `403` → `retryPolicy=abort` (auth is wired by the adapter, not something the LLM can fix; surfaces an actionable operator message). Unexpected statuses default to `retry` to avoid dead-ending a run on a transient quirk we haven't seen yet.
- **22 new unit tests** in `post-issue-interaction.test.ts`: happy-path round-trips for each of the three kinds, optional-field threading (`idempotencyKey`, `title`, `summary`, `continuationPolicy`), `sourceRunId` injection from `PAPERCLIP_RUN_ID` (set / unset / empty), scope-violation rejection (read-only agents get a typed write-scope error before any HTTP call fires), and the full HTTP-status classification matrix described above. Total adapter test count: 360 → 372.

### Notes
- **Wire-format additive only — no prompt-template change, no Hermes-arg change, no marker retirement.** Pre-0.8.21 agents continue to operate exclusively through `post_issue_comment` + RESULT markers as before. The new tool only fires when (a) the agent's `paperclipMcpTools` lists `post_issue_interaction`, AND (b) its prompt explicitly invokes it. Both gates are required so the adapter rollout is fully decoupled from the prompt rollout — we can ship this version to ECS today and configure the LinkedIn Strategist pilot via the MIL repo whenever the agent config flip is ready.
- **PR 3 of the adapter-0.9 phased rollout.** Lands the new tool surface ahead of the 1-week LinkedIn Strategist pilot. PR 4 (`0.9.0-mil.0`) will ship `mil-heartbeat-v4`, flip all 39 agents, and retire the marker convention — gated on this pilot proving the BETA `IssueInteraction` API behaves correctly in production for at least 7 days. If any failure mode surfaces during the pilot, the rollback is a one-line MIL config change (strip the tool from LinkedIn Strategist's `paperclipMcpTools`, agent reverts to marker-only flow on next heartbeat); no adapter rollback required.
- **BETA upstream surface — defensive coding throughout.** The tool only emits the kinds and payload shapes that Paperclip v2026.428.0 actually accepts (verified against `packages/shared/src/validators/issue.ts`). When upstream stabilises the API (likely v2026.43x.0+), additional kinds will land in subsequent adapter releases without breaking this baseline. Any 4xx response is forwarded verbatim to the LLM so a payload-shape shift surfaces as a fixable validation error rather than a silent run failure.

[Full release notes →](https://github.com/marketintellabs/hermes-paperclip-adapter/releases/tag/v0.8.21-mil.0)

## [0.8.20-mil.0] — 2026-05-02

### Added
- **Run-liveness telemetry — adopts the structured shape introduced upstream by Paperclip v2026.428.0 (#4083).** New `result_json` fields on every Hermes run: `livenessState` (`active` / `stalled` / `dead`), `progressBeats[]` (chronological structured events with ISO timestamps), and `nextActionHints[]` (deduped, sorted operator-actionable suggestions derived from observed run behaviour). Dashboards / postmortems / a future Staff Engineer agent can now distinguish three terminal verdicts without parsing run transcripts: a clean run lands as `active` with `[run_start, run_end]` beats and zero hints; a run that brushed its soft-timeout deadline lands as `stalled` with a `soft_timeout_reached` beat and a `"consider raising adapterConfig.timeoutSec"` hint; a hard-timed-out or MCP-crashed run lands as `dead` with a reason hint (`hard_timeout` / `mcp_subprocess_died`) plus an actionable next-step. Implementation lives in `src/server/liveness.ts` as a self-contained `LivenessTracker` class with a clean state machine — `markStalled()` is idempotent (only promotes from `active`), `markDead(reason)` is sticky (overrides `stalled` and supersedes later `markStalled` calls), and `recordHint(hint)` dedupes via a `Set` so the same condition firing across multiple retry attempts surfaces once. Beats and hints are wire-format additive — older Paperclip versions persist the new JSONB fields without acting on them, so this is safe to ship ahead of any consumer-side code. Hooks fire from existing observation points in `execute.ts`: the soft-timeout `setTimeout` callback marks `stalled` and beats `soft_timeout_reached` BEFORE the stderr emission (durable even if the log sink throws); each retry attempt beats `retry_triggered` with the classifier reason; the MCP-died telemetry block calls `markDead("mcp_subprocess_died")` alongside the existing `tool_server_died` errorCode; `result.timedOut` after the runChildProcess loop calls `markDead("hard_timeout")`; transcript-cap truncation, retry success, and unauthorized auto-repair each contribute a hint; the terminal `run_end` beat is recorded last so `result_json` reflects the fully-resolved hint set. The preflight skip path (zero-LLM-cost wakes) lands consistent shape with a single `preflight_skipped` beat so dashboards don't have to special-case it.
- **Optional `adapterConfig.livenessHeartbeatSec` config — periodic `heartbeat_tick` beats while the child is alive.** Default off (`undefined` / `0`). When set to a positive number (clamped to a 5-second floor), the tracker emits a `heartbeat_tick` beat every N seconds with `detail=elapsed=<sec>s` so long-running tool calls (deep web scrapes, large code generation) leave a visible progress trail in `result_json` even when the LLM is silent for minutes between chunks. The interval is `unref()`d so it never holds the event loop open past `stopHeartbeat()`, and `stopHeartbeat()` is invoked in the `runChildProcess` `finally` block so a synchronous throw in the run path can't leak the timer.
- **18 new unit tests** in `liveness.test.ts`: fresh-tracker default state, ISO-timestamp shape, append-order preservation, hint dedupe + sorted snapshot, `markStalled` promotes from active only, `markDead` supersedes stalled and is idempotent across multiple calls, `summary()` returns fresh array copies (mutating consumer arrays cannot corrupt tracker state), `startHeartbeat` is a no-op for zero or negative intervals, `startHeartbeat` is idempotent (second call without stop schedules a single timer), `stopHeartbeat` is safe to call without a prior start and safe to call multiple times, plus four integration-shape tests pinning the wire format for typical successful / soft-timeout / MCP-crash / hard-timeout-supersedes-stalled runs. Total adapter test count: 342 → 360.

### Notes
- **No wire-format breakage, no prompt-template change, no Hermes-arg change.** The new fields are purely additive on `result_json`. Same MCP tool surface, same builtin templates (`mil-heartbeat`, `mil-heartbeat-v2`, `mil-heartbeat-v3`), same retry / transcript-cap / auto-repair behaviour. Safe rolling deploy — agents pinned to 0.8.19-mil.0 and agents pinned to 0.8.20-mil.0 produce semantically equivalent runs; only the telemetry surface differs.
- **Sequencing context: PR 1 of the adapter-0.9 phased rollout.** This release lands the lowest-risk additive piece of the originally-planned `0.9.0-mil.0` bundle (structured interactions adoption + liveness continuations + retire marker convention — see `docs/PLAN_ADAPTER_0.9.md` in the MIL repo). The structured-interactions adoption is gated on stabilising the upstream BETA `IssueInteraction` API and ships separately as `0.8.21-mil.0` (single-agent pilot) followed by `0.9.0-mil.0` (fleet-wide prompt re-roll with marker retirement). Liveness telemetry is independently useful and goes first so the subsequent PRs ship into a baseline where every run already emits `livenessState` — making the smoke validation for `0.9.0-mil.0` substantially easier.
- **Paperclip-side liveness watchdog is already running in production (v2026.428.0).** The upstream watchdog tracks `lastProgressAt` server-side and writes warnings when output streams stall; this adapter release adds the run-side companion fields so the upstream watchdog has structured client observations to correlate with. The optional `livenessHeartbeatSec` is a forward-looking knob — when upstream's `POST /api/runs/:id/liveness` endpoint shape stabilises out of BETA, a future adapter release will gain an HTTP emitter so the watchdog gets in-band liveness signals instead of having to infer from output silence.

[Full release notes →](https://github.com/marketintellabs/hermes-paperclip-adapter/releases/tag/v0.8.20-mil.0)
