...
for the active workstream. Current pin: **`0.9.0-mil.0`** (new builtin `mil-heartbeat-v4` template that documents the BETA `post_issue_interaction` tool and retires the `RESULT:` marker from the prompt surface; both v3 and v4 supported indefinitely). **Also in-flight: `0.9.1-mil.0`**: LLM-API failure classifier — promotes provider failures (HTTP 402/401/403/429 + "failed after N retries") from raw `result_json.result` text to structured `errorCode` + `nextActionHints[]` telemetry. Recent arc: 0.7.x MCP tool server → 0.8.x operational hardening (session-id guards, telemetry, test-mode routing, parallel `create_sub_issues`, skill preload validation, soft-timeout warning, auto-repair detector, retry-with-backoff, transcript cap, `paperclip-hermes-health` CLI, `resolvePromptTemplate` wrapper-prepend defense, run-liveness telemetry, structured issue-thread interactions).
