...
for the active workstream. Current pin: **`0.8.21-mil.0`** (`post_issue_interaction` MCP tool — opt-in writer for Paperclip v2026.428.0's BETA `IssueInteraction` records; registered in the global registry but NOT in any agent's default allowlist; PR 3 of the adapter-0.9 phased rollout). **In-flight: `0.9.0-mil.0`** (PR #28, ready-for-review — Gate A passed via forced LinkedIn Strategist test 2026-05-02): new builtin `mil-heartbeat-v4` template that documents `post_issue_interaction` and retires the `RESULT:` marker from the prompt surface; both v3 and v4 supported indefinitely so the MIL fleet can flip one agent at a time. **Also in-flight: `0.9.1-mil.0`** (PR #29, depends on #28): LLM-API failure classifier — promotes provider failures (HTTP 402/401/403/429 + "failed after N retries") from raw `result_json.result` text to structured `errorCode` + `nextActionHints[]` telemetry; closes the production gap surfaced by the 2026-05-02 fleet audit. Recent arc: 0.7.x MCP tool server → 0.8.x operational hardening (session-id guards, telemetry, test-mode routing, parallel `create_sub_issues`, skill preload validation, soft-timeout warning, auto-repair detector, retry-with-backoff, transcript cap, `paperclip-hermes-health` CLI, `resolvePromptTemplate` wrapper-prepend defense, run-liveness telemetry, structured issue-thread interactions).
